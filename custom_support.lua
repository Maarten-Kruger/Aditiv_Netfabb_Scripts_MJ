...
local function analyze_mesh(mesh)
    ...
    if mesh.trianglecount then tri_count = mesh.trianglecount
    elseif mesh.count then tri_count = mesh.count
    elseif mesh.facecount then tri_count = mesh.facecount
    else
        -- Try method
    ...
    for i = 0, tri_count - 1 do
        ...
        if ok and v1 and v2 and v3 then
            ...
            if dot > 0 then
                ...
                -- Wait, user said "surface critical angle".
                -- Usually defined as angle from Horizontal plane.
                -- Vertical wall = 90 deg from horizontal.
                -- Flat overhang = 0 deg from horizontal.
                -- Support if angle < critical (e.g. 45).

                -- Let's convert:
                -- My calculated 'angle_deg' is from DOWN vector.
                -- Straight down = 0 deg.
                -- Vertical wall = 90 deg.
                -- Horizontal = 90 deg (from down)?? No.

                -- Let's stick to standard overhang angle logic:
                -- Angle theta is between Normal and Down vector.
                -- If theta < CriticalAngle (e.g. 45), it is too steep (too horizontal) and needs support.
                -- Example: Flat ceiling -> Normal is (0,0,-1). Down is (0,0,-1). Angle = 0. 0 < 45 -> Support. Correct.
                -- Vertical wall -> Normal is (1,0,0). Down is (0,0,-1). Angle = 90. 90 > 45 -> No support. Correct.
...
if tray then
    ...
    local items = root.itemcount or 0 -- Fallback if not readable

    log("Scanning " .. items .. " items in tray...")
    ...
    for i = 0, items - 1 do
        ...
        if item then
            ...
            if item.type == "mesh" then mesh = item end -- Hypothetical type check

            -- Try accessing as mesh
            -- Sometimes the item *is* the part which wraps the mesh
            local ok, m = pcall(function() return item:getmesh(0) end)
            if ok and m then mesh = m end

            if mesh then
                analyze_mesh(mesh)
            else
                -- Try treating item as mesh directly
                analyze_mesh(item)
            end
        end
    end
